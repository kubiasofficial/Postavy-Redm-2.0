...
Roku 1899 působí jako muž, který mluví málo, ale když už promluví, lidé poslouchají. Je loajální k těm, na kterých mu záleží, a nebezpečný pro každého, kdo by se jim pokusil ublížit.`,
  ...
  },
  {
    ...
    role: "Lovkyne kozesin / pomocnice / svadlenka",
    age: "22 let",
    origin: "New Austin",
    year: "1899",
    statusText: "Na uteku pred jmenem Blackwood",
    portrait: "images/adelaide.png",
    overview: "Adelaide Rose Blackwood, ktere blizci rikaji Addie, je dcera hledaneho psance Nathaniela Blackwooda. Nikdy nepoznala skutecny domov, jen cestu, prach, strelbu a rozhodnuti, ze nebude stejnym clovekem jako jeji otec.",
    overviewBlocks: [
      { title: "Puvod", text: "Narodila se 12. zari 1877 v New Austinu. Vyrustala mezi taboristi, opustenymi sruby a lidmi, pro ktere zakon koncil tam, kde zacinala sila." },
      { title: "Novy zivot", text: "Cestuje sama, cele jmeno rika jen vyjimecne a nejvice duveruje svemu stribrne sedemu koni Ghostovi. Do West Havenu prichazi s minulosti, kterou nechce opakovat." }
    ],
    lore: `Adelaide Rose Blackwood se narodila 12. zari 1877 v New Austinu. Jeji otec Nathaniel Blackwood patril mezi nejhledanejsi psance na jihozapade. Prepadaval dostavniky, vlaky a obchodni karavany. Nezabijel pro radost, ale nikdy se nezastavil, kdyz mu nekdo stal v ceste. Svet pro nej byl jednoduchy: bud lovis, nebo jsi loven.

Matka Martha Blackwood byvala laskava zena. Po nekolika potratech a smrti prvniho syna vsak propadla laudanu a alkoholu. Nektere dny Adelaide vubec nepoznavala, jine dny sedela u okna a mlcky hledela do krajiny. Presto prave ona naucila Adelaide cist, psat a chovat se slusne.

Adelaide nikdy nepoznala skutecny domov. Jednou spala v jeskyni, podruhe v opustenem srubu, potreti pod plachtou uprostred pouste. Od malicka slysela strelbu, videla krev a videla smrt. Kdyz ji bylo osm let, otec ji daroval maly revolver. Rekl ji, ze ne proto, aby se naucila zabijet, ale proto, aby ji nikdo nezabil.

Nathaniel ji mel rad svym zvlastnim zpusobem. Naucil ji strilet, jezdit, stopovat a prezit. Nikdy ji vsak nenaucil byt ditetem. Matku Adelaide milovala, ale zaroven ji ztracela pred ocima kvuli zavislosti. Prave proto nikdy nevezme do ruky laudan ani jine omamne latky.

Kdyz ji bylo sedmnact let, gang prepadl dostavnik. Jeden z cestujicich se pokusil utect a Nathaniel bez vahani vystrelil. Kulka zasahla mladou matku a vedle ni zustala stat asi sestileta holcicka. Adelaide nikdy nezapomene na jeji plac. Ten vecer pochopila, ze pokud zustane, stane se stejnou jako jeji otec.

Jeste tu noc utekla. Od te doby cestuje sama, nikde nezustava dlouho a nikomu nerika cele sve jmeno. Predstavuje se jen jako Adelaide. Prijmeni Blackwood vyslovi jen tehdy, kdyz uz nema jinou moznost. Na levem zapesti ma vypalene male pismeno B, stejnou znacku, kterou nosili clenove Blackwoodova gangu. Vetsinu casu ji schovava pod rukavem.`,
    appearance: "Mlada zena s opatrnym pohledem, svetle hnedymi az rezavymi vlasy a praktickym tmavym oblecenim. Nosi tmavou halenku, vestu nebo kabat, kozeny opasek, jezdecke boty a rukavice. Na cestach pres ramena nosi kozesinovy plast.",
    personality: "Na prvni pohled pusobi chladne, ve skutecnosti je jen velmi opatrna. Nejdrive dlouho pozoruje a az potom zacne verit. Kdyz si nekoho pusti k sobe, udela pro nej cokoliv.",
    quote: "\"Cloveka neurcuje krev v jeho zilach... ale rozhodnuti, ktera udela.\""
  },
  {
    id: "silas",
...
const retiredCharacters = [
  ...
  },
  {
    id: "eleanor",
    discordId: "1454130138240520407",
    playerLabel: "Hrac Ellie / Adelaide",
    listName: "Eleanor",
    name: "Eleanor \"Ellie\" Whitmore",
    role: "Empaticka vypravecka / opora / novy zacatek",
    portrait: "images/elie.png",
    status: "Zesnula",
    totalMs: (334 * 60 + 13) * 60 * 1000,
    epitaph: "Eleanor Ellie Whitmore stravila ve West Havenu 334h 13min. Jeji klid, empatie a tichy novy zacatek zustavaji zapsane v rodinnem archivu."
  }
];
...
const migrateActiveCharacters = (list) => {
  let normalized = (Array.isArray(list) ? list : [])
    ...
    .filter((character) => !["violet", "eleanor"].includes(character.id));

  const requiredCharacters = ["scarlet", "adelaide"]
    .map((id) => cloneCharacter(defaultCharacters.find((character) => character.id === id)))
    .filter(Boolean);

  requiredCharacters.forEach((required) => {
    const hasRequired = normalized.some((character) => character.id === required.id);
    if (hasRequired) {
      normalized = normalized.map((character) => (
        character.id === required.id
          ? normalizeCharacter({ ...required, ...character, discordId: required.discordId })
          : character
      ));
      return;
    }

    const defaultIndex = defaultCharacters.findIndex((character) => character.id === required.id);
    const previousDefaultIds = defaultCharacters
      .slice(0, Math.max(defaultIndex, 0))
      .map((character) => character.id);
    const insertAt = Math.max(
      0,
      ...previousDefaultIds.map((id) => normalized.findIndex((character) => character.id === id) + 1)
    );
    normalized.splice(insertAt, 0, required);
  });

  return normalized.filter((character, index, all) => (
...
const boot = async () => {
  ...
  if (session.characterId === "eleanor") {
    session.characterId = "adelaide";
  }
